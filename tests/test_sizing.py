...
class TestTargetUnits:
    def test_vol_targeting_formula(self):
        # 100k * 2% / 1% = 200k di esposizione (sotto il cap 400k) -> 2000 azioni a $100
        assert target_units(100_000, 0.01, 100.0) == 2000

    def test_leverage_cap_binds(self):
        # 100k * 2% / 0.1% = 2M -> cap a 4x = 400k -> 4000 azioni a $100
    ...
    def test_futures_notional(self):
        # ES: notional = prezzo * 50; 100k*2%/1% = 200k -> floor(200k/250k) = 0
        assert target_units(100_000, 0.01, 5000.0 * 50) == 0
        # con equity 1M: 2M -> cap 4M no; 2M/250k = 8 contratti
...
class TestDailySigma:
    def test_no_lookahead_shift(self):
        ...
        # primo valore disponibile: dopo 3 rendimenti (giorni 1..3) + shift -> giorno 4
    ...
    def test_matches_manual_std(self):
        ...
        expected = rets.iloc[15:29].std()  # finestra che termina al giorno 29-1
