"""End-to-end smoke test: the full pipeline on a synthetic random walk."""
...
def random_walk_bars():
    ...
    days, price = [], 100.0
    for d in dates:
        # overnight gap + intraday random walk
...
def test_full_pipeline_runs(random_walk_bars):
    ...
    for mode in ("base", "final"):
        ...
        assert not res.trades.empty, f"no trades in {mode} mode"
        # accounting: final equity = initial + sum of net pnl
        assert res.equity.iloc[-1] == pytest.approx(
            100_000.0 + res.trades["net_pnl"].sum()
        )
        # always flat overnight: every trade opens and closes the same day
        same_day = (
            res.trades["entry_time"].dt.normalize()
            == res.trades["exit_time"].dt.normalize()
        )
        assert same_day.all()
        # no trades during the warmup days (14d sigma + 15d daily vol)
    ...
    # the final variant exits at the same time or earlier, never later than
    # base (same entry -> final exit <= base exit, by trailing construction)
    ...
    assert len(yt) == 1  # a single year in the synthetic sample


def test_final_never_exits_later_than_base(random_walk_bars):
    """For the same entry, the 'final' trailing stop is tighter than 'base'."""
