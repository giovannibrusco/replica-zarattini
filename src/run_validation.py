"""Esegue l'intero piano di validazione e genera il report markdown.
...
def _fmt_perf(perf: dict, ts: dict | None = None) -> dict:
    row = {
        ...
        "CAGR": f"{perf['cagr']:+.2%}",
        "Vol ann.": f"{perf['ann_vol']:.1%}",
        "Sharpe": f"{perf['sharpe']:.2f}",
        "Max DD": f"{perf['max_drawdown']:.1%}",
    }
    if "alpha_ann" in perf:
        row["Alfa ann."] = f"{perf['alpha_ann']:+.1%}"
        row["t-stat alfa"] = f"{perf['alpha_tstat']:.2f}"
...
def main() -> None:
    ...
    buf = io.StringIO()
    w = buf.write
    w("# Validazione replica — SPY (dati Alpaca IEX)\n\n")
    days = rth.index.normalize().unique()
    w(f"Campione: **{days[0].date()} → {days[-1].date()}** "
      f"({len(days)} giorni, {len(rth):,} barre 1-min RTH).\n\n")
    ...
    w("## Base vs Final vs benchmark passivo\n\n")
    w(pd.DataFrame(rows).fillna("—").to_markdown() + "\n\n")

    w("Benchmark esterni attesi: paper (SPY 2007-24) Sharpe ~1.33, alfa ~19.6%, "
      "beta ~0; replica Quantitativo (ES) ~+2 bps/trade, win rate ~36%, "
      "payoff ~2.1.\n\n")

    # --- per anno ---
    w("## Robustezza temporale (variante final)\n\n")
    yt = yearly_table(results["final"].daily_returns, spy_ret)
    ytb = yearly_table(spy_ret)
    tab = pd.DataFrame(
        {
            "Strategia": yt["total_return"].map("{:+.1%}".format),
            "Sharpe": yt["sharpe"].map("{:.2f}".format),
            "Alfa ann.": yt["alpha_ann"].map("{:+.1%}".format),
    ...
    if os.path.exists(args.vix):
        ...
        w("## Regimi VIX (variante final)\n\n")
        rtab = pd.DataFrame(
            {
                "Total return": rt["total_return"].map("{:+.1%}".format),
                "Sharpe": rt["sharpe"].map("{:.2f}".format),
                "Giorni": rt["n_days"].astype(int),
            }
        )
        w(rtab.to_markdown() + "\n\n")

    # --- sensitivity costi ---
    w("## Sensitivity slippage (variante final)\n\n")
    srows = {}
    ...
    w(
        "## Caveat\n\n"
        "- **Feed IEX** (~3% del volume): VWAP approssimato (tocca la exit "
        "final) e minuti mancanti sparsi (il motore usa l'ultimo prezzo "
        "disponibile al check).\n"
        "- **Campione dal 2020-07** (limite free tier Alpaca): niente 2008 "
        "ne' periodo pre-COVID; il confronto col paper (2007-24) e' quindi "
        "di ordine di grandezza.\n"
        "- Commissioni IB $0.0035/share (min $0.35) come nel paper; "
        "slippage base 0.\n"
